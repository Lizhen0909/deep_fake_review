...
def read_dataset(input_list_file):
    ...
    targets = np.array([i%2 for i in range(len(paths))])
...
if __name__ == '__main__':
    ...
    images, targets = read_dataset(config.input)
    scores = []
    for i,(img_p,target) in enumerate(zip(images,targets)):
